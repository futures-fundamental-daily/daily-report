...
def calculate_fundamental_score(code, quote_data, stock_data=None):
    """计算基本面评分（含关联股票板块联动）"""
    ...
    # 宏观环境 (20%)
    macro = knowledge.get("macro_factor", "")
    if "支撑" in macro or "利好" in macro:
        score += 0.5
    if "压制" in macro or "放缓" in macro:
        score -= 0.5
    ...
    if stock_data and code in stock_data:
        ...
        if change > 1 and sector_change > 1:
            ...
            score -= 0.3  # 期货跌 + 股票板块跌（确认弱势）
        # 期货与股票背离警示
        elif change > 1 and sector_change < -1:
            score -= 0.5  # 期货涨但股票板块跌，警惕假突破
        elif change < -1 and sector_change > 1:
            score += 0.3  # 期货跌但股票板块涨，可能超跌
...
def generate_logic(code, quote_data, knowledge, score, momentum):
    """生成AI分析逻辑"""
    logic = []
    price = quote_data.get("close", 0)
    change = quote_data.get("change_pct", 0)
    ...
    if position < 20:
        ...
        logic.append(f"价格处于历史中位区间（{position:.0f}%），方向待定")
...
def generate_risk(code, quote_data, knowledge):
    """生成风险提示"""
    risks = []
    change = quote_data.get("change_pct", 0)
    price = quote_data.get("close", 0)
    ...
    # 持仓减少+价格上涨
    if change > 1 and oi > 0 and oi < quote_data.get("prev_oi", oi):
        risks.append("持仓减少但价格上涨，多头获利了结信号")
...
def analyze_all():
    ...
    for p in products:
        ...
        # 传入stock_data进行板块联动评分
        score, momentum = calculate_fundamental_score(code, quote, stock_data)
        ...
        logic = generate_logic(code, quote, knowledge, score, momentum)
        risks = generate_risk(code, quote, knowledge)
